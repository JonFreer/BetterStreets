...
function PopUp(props: { open: boolean, id: string, data: any, closeCallback: any, updateCallback: any }) {
    ...
    React.useEffect(() => {
        ...
        if (isActive && isPaused === false) {
            interval = setInterval(() => {
                if (stage == 0) {
                    setWaitingTime((time) => time + 100);

                } else {
                    setCrossingTime((time) => time + 100);
    ...
    function submit() {
        ...
        const requestOptions = {
            ...
            body: JSON.stringify({ id: props.id, waiting_time: input_waiting_time, crossing_time: input_crossing_time, notes: notes })
        };
